...
function createSection() {
    ...
    section.innerHTML = `
        <div class="header" data-v-c41b640a>
...
function setupTransparentButtons() {
    const styleId = 'buttonNewStyle';

    // Проверяем, сохранён ли стиль в localStorage
    ...
    document.querySelector('.zxc7').addEventListener('click', function() {
        ...
        if (existingStyle) {
            ...
            localStorage.setItem('isStyleApplied', 'false'); // Убираем стиль и сохраняем это в localStorage
        } else {
            applyStyle(styleId);
            localStorage.setItem('isStyleApplied', 'true'); // Применяем стиль и сохраняем это в localStorage
        }
    });
}

// Эта функция должна находиться вне функции setupTransparentButtons
